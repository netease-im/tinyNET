...
int wrapSocket(tnet_transport_t *transport, transport_socket_xt *sock)
{
    ...
    if (TNET_SOCKET_TYPE_IS_DGRAM(sock->type)) {
        ...
        if (!sock->cf_read_stream && !sock->cf_write_stream) {
            ...
            CFReadStreamSetProperty(sock->cf_read_stream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeBackground);
            CFWriteStreamSetProperty(sock->cf_write_stream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeBackground);
